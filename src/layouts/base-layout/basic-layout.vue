...
const contentHeightVar = computed(
  () =>
    `calc(100vh - ${props.layout === 'side' ? '0px' : props.headerHeight}px)`
)
...
const headerFixedStyle = computed(() => ({
  ...
  right: 0,
  zIndex: 1,
  paddingLeft: props.headerFixed
    ? `calc(${
        props.collapsed ? props.siderCollapsedWidth : props.siderWidth
      }px + 1rem)`
    : 'pl-4'
}))
const onShow = () => {
  ...
  <LayoutBase :has-sider="baseSider" class="h-screen">
    ...
    >
      <div class="flex items-center">
        <Logo :src="logo" />
        <n-icon v-if="isMobile" size="24" class="ml-12px" @click="onShow">
          <MenuUnfoldOutlined />
        </n-icon>
    ...
    <LayoutBase :has-sider="innerSider" class="pro-admin-mix-layout-content">
      ...
      <div v-if="headerFixed" class="pro-admin-mix-layout-header"></div>
      <LayoutHeader
        v-if="layout === 'side' && !isMobile"
    ...
    >
      <n-drawer-content body-content-style="padding: 0">
        <n-layout class="h-100%">
          <n-layout-header class="h-60px" inverted>
            <div
              v-if="logoVisible"
          ...
          <n-layout-content style="height: calc(100% - 60px)">
            <n-el tag="div" class="bg-[var(--inverted-color)] h-100%">
              <SideMenu
                :inverted="true"
                :value="active"
                :options="options"
